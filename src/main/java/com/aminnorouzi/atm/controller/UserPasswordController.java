...
public class UserPasswordController implements Initializable {
    ...
    private void handleSave(ActionEvent event) throws IOException {
        ...
        if (settings.validateUserPassword(currentPassword)) {
            settings.changeUserPassword(newPassword);
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        saveButton.disableProperty().bind(currentPasswordField.textProperty().isEmpty()
                ...
                .or(currentPasswordField.textProperty().length().greaterThan(32))
                .or(newPasswordField.textProperty().length().greaterThan(32))
                .or(confirmPasswordField.textProperty().length().greaterThan(32))
